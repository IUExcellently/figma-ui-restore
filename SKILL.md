...
description: Use when restoring Figma MCP data, screenshots, node metadata, or UI descriptions into maintainable frontend code. Supports platform routing for mini-program, H5 / mobile web, and Web PC admin. Always determine target platform first, then load common rules and platform-specific rules.
---
...
当前版本通过平台 Profile 支持小程序、H5 / 移动 Web、Web PC 后台三类 UI 还原。
...
Web PC 后台规则最成熟，小程序与 H5 通过 `platforms` 目录提供专属约束。命中 Skill 后，必须先判断目标平台，不得默认套用 Web PC 后台规则。平台规则优先级高于公共规则。
...
目标平台为 `mini-program` 时，先读取 `steering/platforms/mini-program/README.md`。推荐按阶段加载：
