...
- `steering/`：平台路由、公共规则和平台专属规则，也供 Kiro / Codex / Qoder 读取
- `references/`：输入模板、自检清单、常量模板、截图验收模板
- `.codex/INSTALL.md`：Codex 快速安装说明
- `.qoder/rules/`：Qoder 通用平台规则与 Web PC 后台兼容规则
...
项目级通用规则可以复制 `.qoder/rules/figma-ui-restore.md` 到目标项目；仅 Web PC 后台项目可复制 `.qoder/rules/figma-backend-admin-ui-restore.md`。
