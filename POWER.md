...
2. 单尺寸 / 多尺寸输入判断
3. Layout 边界判断
4. 本次生成范围：Layout / Content / 组件 / 局部模块
5. 项目上下文扫描计划
6. 项目技术栈与可复用组件判断
7. 多尺寸源数据扫描结果（如存在）
8. 基准稿 / 参考稿 / 状态变体归类（如存在多尺寸或多状态）
9. 设计 token 映射思路
10. 组件拆分思路
11. ViewModel 数据模型草案
12. 语义化事件设计
13. 响应式策略待确认点
14. Sidebar / Menu / Header 响应式归属确认
15. 资源处理方案
16. Icon 尺寸识别方案
17. 表格 / 表单 / 列表区域处理方案
18. 风险点与待确认事项
...
* 多尺寸源数据是增强输入，不是必需输入；单页面单尺寸 UI 继续沿用原有单稿还原流程。
* 单页面多尺寸 UI 以最大宽度的默认有数据态为基准开发；如果最大尺寸只有 empty / loading / error 状态，必须列为待确认项。
* 多页面多尺寸输入必须先按业务页面分组，不能只按尺寸分组；需结合 pageName、frameName、nodeId、目录名、截图命名、`meta.json` source 信息判断是否属于同一页面。
* 同尺寸 empty / loading / error 等差异属于状态变体，不属于响应式断点；多尺寸里的样例数据差异不能写成媒体查询逻辑。
